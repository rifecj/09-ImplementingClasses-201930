...
class Point(object):
    ...
    def move_to(self,x,y):
        self.distance_traveled = self.distance_traveled + math.sqrt((self.x - x) ** 2 + (self.y - y) ** 2)
        self.x=x
        self.y=y
        self.number_of_moves_made=self.number_of_moves_made+1

    def move_by(self,dx,dy):
        self.distance_traveled = self.distance_traveled + math.sqrt((self.x - self.x+dx) ** 2 + (self.y - self.y+dy) ** 2)
        self.x=self.x+dx
        self.y=self.y+dy
        self.number_of_moves_made = self.number_of_moves_made + 1
    ...
    def closer_to(self,p1,p2):
        dist_p1=math.sqrt((self.x-p1.x)**2+(self.y-p1.y)**2)
        dist_p2=math.sqrt((self.x-p2.x)**2+(self.y-p2.y)**2)
        if dist_p1<=dist_p2:
            return p1
        else:
            return p2
    ...
    def halfway_to(self,p1):
        half_to_x=(self.x-p1.x)/2+p1.x
        half_to_y = (self.y - p1.y) / 2 + p1.y
        half_p=Point(half_to_x,half_to_y)
        return(half_p)
